...
pub(crate) fn parse_inference_profiles_page(
    ...
    let models = parsed
        ...
        .collect();
    Ok(InferenceProfilePage {
        models,
        next_token: parsed.next_token.filter(|t| !t.trim().is_empty()),
    })
}
...
mod tests {
    ...
    #[test]
    fn surfaces_next_token_for_pagination() {
        let body = r#"{
            "inferenceProfileSummaries": [
        ...
        let page = parse_inference_profiles_page(body).expect("parse");
        assert_eq!(page.models.len(), 1);
        assert_eq!(page.next_token.as_deref(), Some("page-2-token"));
    }

    #[test]
    fn blank_next_token_is_treated_as_last_page() {
        let body = r#"{"inferenceProfileSummaries": [], "nextToken": "  "}"#;
        let page = parse_inference_profiles_page(body).expect("parse");
        assert!(
            page.next_token.is_none(),
            "blank token must stop pagination"
        );
    }
}
