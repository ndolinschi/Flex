...
//! tools: both tools need "the up-to-date index for this session's cwd".
//!
//! `IndexStore::build` is itself incremental (it diffs against the persisted
//! manifest and only re-processes changed files), so there is no separate
//! cache of open stores to maintain: every tool call opens the store (cheap
//! — a manifest/symbol JSON read plus an mmap handle) and calls `build()`
//! (cheap when nothing changed, does real work on first use or after edits).
...
/// Open (or create) the index rooted at `cwd` and bring it up to date, at the
/// given `index_dir`. The actual worker behind [`open_and_build`]; split out
/// so tests can pass an explicit scratch `index_dir` instead of depending on
