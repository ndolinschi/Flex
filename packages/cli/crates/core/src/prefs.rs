...
pub struct CliPrefs {
    ...
    /// Whether the TUI captures the mouse. `Some(true)` (the default) makes the
    /// wheel scroll the transcript reliably on every terminal; `Some(false)`
    /// releases the mouse so click-drag is native text selection (at the cost
    /// of the wheel scrolling the terminal's own buffer where 1007 is ignored,
    /// e.g. Apple Terminal.app). Persisted when the user hits Ctrl+M; `None` =
    /// use the default.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// Model-call iterations allowed per turn before the engine stops the
    /// turn on its own; `None` keeps the engine default. Raise this for
    /// long, autonomous multi-file build tasks that legitimately need more
    /// than the default number of model round trips in one turn.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub max_iterations: Option<u32>,
    /// Effort level (`"low"`, `"medium"`, `"high"`, `"xhigh"`, `"max"`); how
    /// hard the agent works — scales reasoning depth, tool aggressiveness, and
...
/// The role overrides that make up the DeepSeek orchestration preset
/// (pure, no I/O): fast `searcher`, strong `worker`. Models-only overrides —
/// `role_specs` seeds tools/prompt from the same-name built-ins.
pub fn deepseek_roles_preset() -> BTreeMap<String, RoleConfig> {
    BTreeMap::from([
        (
            "searcher".to_owned(),
            RoleConfig {
                models: vec!["deepseek/deepseek-v4-flash".to_owned()],
                ..RoleConfig::default()
            },
        ),
        (
            "worker".to_owned(),
            RoleConfig {
                models: vec!["deepseek/deepseek-v4-pro".to_owned()],
                ..RoleConfig::default()
            },
        ),
    ])
...
impl CliPrefs {
    ...
    /// Update and persist the effort level.
    pub fn remember_effort(level: &str) -> Result<(), PrefsError> {
        let mut prefs = Self::load();
        prefs.effort = Some(level.to_owned());
        prefs.save()
    }
    ...
    /// Configure DeepSeek multi-agent orchestration and persist: research
    /// subagents (`searcher`) run the fast `deepseek-v4-flash`, implementation
    /// subagents (`worker`) run the strong `deepseek-v4-pro` — the DeepSeek
    /// analog of Claude Code's Haiku-search / Sonnet-work split. The main
    /// (planner) session model is set by the caller. Overwrites any prior
    /// `searcher`/`worker` overrides; other roles are left untouched.
    pub fn apply_deepseek_roles_preset() -> Result<(), PrefsError> {
        let mut prefs = Self::load();
        for (name, config) in deepseek_roles_preset() {
            prefs.roles.insert(name, config);
        }
        prefs.save()
    }
...
mod tests {
    ...
    fn max_iterations_round_trip() {
        ...
        CliPrefs::save_to(&path, &prefs).expect("save");
        let loaded = CliPrefs::load_from(&path).expect("load");
        assert_eq!(loaded.max_iterations, Some(500));
    }
    ...
    fn effort_round_trip() {
        ...
        CliPrefs::save_to(&path, &prefs).expect("save");
        let loaded = CliPrefs::load_from(&path).expect("load");
        assert_eq!(loaded.effort.as_deref(), Some("max"));
    }
    ...
    #[test]
    fn deepseek_preset_assigns_flash_search_and_pro_worker() {
        let preset = deepseek_roles_preset();
        assert_eq!(
            preset["searcher"].models,
            vec!["deepseek/deepseek-v4-flash".to_owned()]
        );
        assert_eq!(
            preset["worker"].models,
            vec!["deepseek/deepseek-v4-pro".to_owned()]
        );
        // role_specs seeds tools/prompt from the built-ins: a models-only
        // preset keeps searcher read-only and worker full.
        let prefs = CliPrefs {
            roles: preset,
            ..CliPrefs::default()
        };
        ...
        let searcher = specs
            .iter()
            .find(|s| s.name == "searcher")
            .expect("searcher");
        assert_eq!(
            searcher.models,
            vec![ModelRef::from("deepseek/deepseek-v4-flash")]
        );
        assert_eq!(searcher.tools, RoleToolProfile::ReadOnly);
        let worker = specs.iter().find(|s| s.name == "worker").expect("worker");
