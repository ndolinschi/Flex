...
pub enum LocalCommand {
    ...
    /// `/effort [low|medium|high|xhigh|max]` — how hard the agent works.
    Effort { arg: Option<String> },
    ...
    Providers,
    /// `/roles` — list orchestration roles; `/roles preset <name>` applies a
    /// named role/model preset (e.g. `deepseek`).
    Roles { preset: Option<String> },
...
fn parse_roles(args: &str) -> LocalCommand {
    let trimmed = args.trim();
    // `/roles preset <name>` — take only the first token after `preset` (so
    // `preset deepseek extra` reports `deepseek`, not the whole tail), and
    // require a word boundary so `presetfoo` isn't parsed as `foo`.
    let preset = trimmed
        .strip_prefix("preset")
        .filter(|rest| rest.is_empty() || rest.starts_with(char::is_whitespace))
        .map(|rest| rest.split_whitespace().next().unwrap_or("").to_owned());
    LocalCommand::Roles { preset }
}
...
mod tests {
    ...
    #[test]
    fn route_roles_and_roles_preset() {
        let index = CommandIndex::default();
        assert_eq!(
            index.route("/roles"),
            Route::Local(LocalCommand::Roles { preset: None })
        );
        assert_eq!(
            index.route("/roles preset deepseek"),
            Route::Local(LocalCommand::Roles {
                preset: Some("deepseek".to_owned())
            })
        );
        // Trailing tokens must not leak into the preset name.
        assert_eq!(
            index.route("/roles preset deepseek extra garbage"),
            Route::Local(LocalCommand::Roles {
                preset: Some("deepseek".to_owned())
            })
        );
