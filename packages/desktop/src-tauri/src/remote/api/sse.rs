...
//!
//! Unlike `flex serve`, this stream **includes** streaming deltas so a mobile
//! client can render live tokens.

use axum::response::sse::{Event, KeepAlive, Sse};
use futures::stream::{Stream, StreamExt};

use agentloop_contracts::SessionEvent;
...
> {
    ...
    let stream = futures::stream::iter(backlog).chain(live).map(to_sse_event);
