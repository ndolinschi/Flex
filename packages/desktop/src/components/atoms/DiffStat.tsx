 ...
 *  - raw integer counts (these are line counts, not tokens — no `formatTokens`
 *    k-suffix)
 *  - green `+`, red `−`
...
export const DiffStat = ({
  ...
  return (
    ...
    >
      {formatted.added > 0 ? <span className="text-green">+{formatted.added}</span> : null}
      {formatted.removed > 0 ? (
        <span className="text-red">−{formatted.removed}</span>
      ) : null}
    </span>
  )
}
