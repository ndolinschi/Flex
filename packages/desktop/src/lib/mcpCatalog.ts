...
export type McpCatalogEnvKey = {
  /** Environment variable name, written verbatim into `McpServerDto.env`. */
  name: string
  /** Human label shown above the input in the install dialog. */
  label: string
  /** Masks the input (password-style) and is never pre-filled/echoed. */
  secret: boolean
  required: boolean
  placeholder?: string
}
...
export type McpCatalogArgKey = {
   ...
   * folded into `args` at install time per `argsTemplate`). */
  key: string
  label: string
  required: boolean
  placeholder?: string
