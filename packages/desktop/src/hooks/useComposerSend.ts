...
export const useComposerSend = ({
  ...
  const handleStop = async () => {
    ...
    // Sticky Stop: a wedged turn (e.g. a blocking FOREGROUND command that
    // never exits) keeps emitting live deltas that would immediately re-arm
    // isStreaming via applyGlobalEvent's straggler-guarded re-arm sites — so
    // the button snaps back to "working" and every new message queues, making
    // Stop look like a no-op. Record a completion marker (sentinel — no
    // turn_id) so `isStragglerForCompletedTurn()` treats every subsequent
    // live event as a straggler: the button stays "Send" and the queue can
    // drain. A real `turn_started` (the next prompt) clears this via
    // `clearCompletedTurn`, so it can never mute a genuinely-new turn.
