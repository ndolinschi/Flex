...
export const useSessionEvents = (
  ...
  useEffect(() => {
    ...
    const boot = async () => {
      ...
      // Warm remount: same session already folded — reattach bus and only
      // pull events after lastSeq (cache hit; no full JSONL fold).
      if (hasBootedRef.current) {
        ...
        try {
          ...
          if (fromSeq != null) {
            ...
            if (deltas.length > 0) {
              const { accumulated, buffers, spans } = applyReplayEvents(
                deltas,
                { resetTotals: false, seedAccumulated: true },
              )
              const swept = closeRunningRows(accumulated)
              rowsRef.current = swept
              setRows(swept)
