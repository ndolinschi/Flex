...
| `RightPanel` | Plan / Changes / Files / Terminal / Browser / Memory (flagged) / plugin tabs (Database); tabs under `organisms/right-panel/` (`RightPanelTabBar`, `RightPanelMiniTabs`, `tabs`) + `src/plugins/` registry. Closed by default on app start and New Agent (`setActiveSessionId(…, { panel: "closed" })`); opens via `+`, ⌘J, Plan mode, session switch restore, or the closed-panel mini-tabs flyout. Memory gated by `MEMORY_TAB_ENABLED` (default off). Database via UI plugin (`DATABASE_TAB_ENABLED`, default on). | — | App shell |
| `RightPanelMiniTabs` | Cursor-style borderless mini rows when the right panel is closed on chat: session open tabs plus pinned Changes / Browser / Terminal / Files (deduped). Click opens the full panel on that tab; Changes shows `DiffStat`; Terminal shows PTY count. No section labels or expand control. | `openTabDefs`, `selectedTab`, `changesTotals`, `terminalCount`, `catalog`, `onSelectTab` | RightPanel |
