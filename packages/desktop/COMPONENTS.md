...
| `FilesTab` | Open-file strip + Monaco editor with **inline completions** (prompt-completion plugin) and a collapsible **Problems** strip (`useMonacoMarkers` — TS/JS/JSON/CSS/HTML workers); `.md`/`.mdx` default to `MarkdownBody` preview (Code/Eye toggle); empty/browse shows `FileExplorer`. | `active` | ToolTabBody |
| `ArtifactsTab` | UI plugin: project-scoped shelf of AI-created non-code deliverables (csv / spreadsheet / presentation / diagram / image / …). Left list with **agent affinity** (“This agent” vs other session); CSV + image preview in-pane; other kinds open externally. Auto-registers from Write/Edit of allowlisted paths under `artifacts/`/`reports/`/… via `artifactSideEffects`. | `active`, `session` | ToolTabBody (plugin registry) |
