...
| `Tab` | Pill tab / open-buffer chip; pointer DnD | `selected`, `size?`, `onSelect`, `onClick?`, `onClose?`, `groupColor?`, `activityDot?`, `sessionColor?`, `rangeSelected?`, … | ContentPane |
...
| `ThemeLibrary` | Custom color themes — select/import JSON/export/delete named themes; visual token editor for a curated subset; apply allowlisted overrides as inline CSS vars | — | Settings Appearance |
...
| `DiffView` | Unified-diff renderer; hunk Keep/Undo when enabled; **N unmodified lines** collapse bars between/before hunks | `diff`, `onKeepHunk?`, `onUndoHunk?`, `collapseUnmodified?` | FileRow, PrTab |
...
| `WorkingAgentsPill` | Composer-adjacent **"N Working"** card (worker titles + Jump) with **Stop All** (cancels parent turn) | `workers`, `onScrollToWorkers?`, `onStopAll?` | ChatSessionBody → Composer `workersSlot` |
...
| `ContentWorkspace` | One or two content panes (optional split sash; first split starts ≈48% chat / 52% work; active sessions open Changes beside chat by default); TabStrip is the topmost header; tab DnD ghost + cross-pane drop zones | `onOpenCommandPalette?`, `onOpenSearch?` | App shell |
...
| `FilesTab` | Workspace **file explorer only** (Cursor Agents). Tree click / Diff Open / path click create a dedicated `kind: "file"` document tab in the content TabStrip (`openWorkspaceFile` → `openFileBesideChat`); multiple file tabs; drag across panes via existing tab DnD. | `active`, `session` | ToolTabBody |
| `FileDocumentTab` | Document body for a content `file` tab: path breadcrumbs, Monaco editor (+ inline completions), `.md`/`.mdx` **Preview / Source**, Problems strip, Save. | `path`, `session`, `active` | ContentPane |
| `ArtifactsTab` | UI plugin: project-scoped shelf of AI-created non-code deliverables. Left list with agent affinity; CSV + image preview; **Open with** context/overflow (Artifacts / file tab / Files / Folder / external / Browser when backends allow). Auto-registers via `artifactSideEffects`. | `active`, `session` | ToolTabBody (plugin registry) |
