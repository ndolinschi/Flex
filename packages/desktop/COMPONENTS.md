...
| `ContentWorkspace` | One or two content panes (optional split sash); no secondary header | — | App shell |
| `ContentPane` | Tab strip for chat + tool tabs; pointer DnD; `+` menu; chat bodies mount on first visit (not every open tab) | `paneIndex`, `keepAliveTools` | ContentWorkspace |
