...
| `FilesTab` | Open-file strip + path breadcrumbs + Monaco (`bg-editor`) with **inline completions**, toggleable compact right-side `FileExplorer` (open by default; accent edge on active row), collapsible **Problems** strip; `.md`/`.mdx` default to preview. | `active` | ToolTabBody |
