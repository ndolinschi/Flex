...
| `FilesTab` | Open-file strip + Monaco editor with **inline completions** (prompt-completion plugin), 8px Monaco scrollbars, toggleable compact right-side `FileExplorer` with active-buffer highlight, and a collapsible **Problems** strip (`useMonacoMarkers` — TS/JS/JSON/CSS/HTML workers); `.md`/`.mdx` default to `MarkdownBody` preview (Code/Eye toggle); an empty Files tab shows the explorer full-width. | `active` | ToolTabBody |
