...
| `AppHeader` | Split toggle (⌘J), session menu | — | ContentWorkspace |
...
| `WindowTitleBar` | Compact custom window chrome (`decorations: false`, 30px): traffic lights / caption buttons + sidebar toggle + drag region (double-click zooms — fullscreen on macOS, maximize elsewhere); in-window File/Edit/View/Help on Windows/Linux; native macOS menu bar via `useNativeAppMenu`; macOS corners clipped natively to 10px | `onOpenCommandPalette?`, `onOpenSearch?` | App shell |
